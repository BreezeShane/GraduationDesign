...
def train(args):
    ...
    for epoch in trange(TRANSFER_TRAIN_CFG.EPOCHS, desc="Training Submodel Epoch: "):
        model.train()
        train_epoch_loss = []
        acc, nums = 0., 0

        for idx, (labels, inputs) in enumerate(tqdm(t_dataloader, desc="Training Submodel: ")):
        ...
        with torch.no_grad():
            model.eval()
            for vidx, (labels, inputs) in enumerate(tqdm(v_dataloader, desc="Validating submodel: ")):
                inputs = inputs.to(args.device)
                labels = labels.to(args.device)
                ...
                #TB Print valid loss
                writer.add_scalar(f"Submodel_V_Loss_BS_{vidx}_Epoch_{epoch+1}", loss.item(), vidx)
        v_loss_avg = np.average(valid_epoch_loss)
        v_avg_acc = 100 * v_acc / v_nums
        valid_epochs_loss.append(v_loss_avg)
        valid_epochs_acc.append(v_avg_acc)
        print("\n----------------------------------------------------------------------------------")
        print(f"| Epoch: {epoch+1}, Valid Acc: {v_avg_acc:.3f}%, Avg Loss: {v_loss_avg} |")
        print("----------------------------------------------------------------------------------\n")
